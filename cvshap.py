...
# Create additional 100-sample subset for SHAP methods
print("\nCreating 100-sample subset for SHAP methods...")
shap_sample_size = min(100, X_reduced.shape[0])
np.random.seed(43)
shap_sampled_indices = np.random.choice(X_reduced.shape[0], size=shap_sample_size, replace=False)
X_shap = X_reduced.iloc[shap_sampled_indices].copy()
y_shap = y_reduced.iloc[shap_sampled_indices].copy()
print(f"SHAP subset: {X_shap.shape[0]} samples")
...
# ─────────────────────────────────────────────
# 6. RF-SHAP (using 100 instances)
# ─────────────────────────────────────────────
print("\nRunning RF-SHAP Feature Selection (100 instances)...")
...
rf_shap_idx = np.argsort(mean_abs_shap_rf)[::-1]
top5_rf_shap_features = X_shap.columns[rf_shap_idx[:5]].tolist()

# CV5 evaluation
X_cv5_rf_shap = X_reduced[top5_rf_shap_features]
cv5_rf_shap_score = cross_val_score(rf_shap, X_cv5_rf_shap, y_reduced, cv=5, scoring='r2').mean()

# CV4: drop top feature and recalculate
reduced_fullset_shap = X_shap.drop(top5_rf_shap_features[0], axis=1)
y_shap_reduced = y_shap.copy()

rf_shap2 = RandomForestRegressor(n_estimators=100, random_state=42)
rf_shap2.fit(reduced_fullset_shap, y_shap_reduced)

explainer_rf2 = shap.TreeExplainer(rf_shap2)
shap_values_rf2 = explainer_rf2.shap_values(reduced_fullset_shap)

mean_abs_shap_rf2 = np.abs(shap_values_rf2).mean(axis=0)
rf_shap_idx2 = np.argsort(mean_abs_shap_rf2)[::-1]
top4_rf_shap_features_reduced = reduced_fullset_shap.columns[rf_shap_idx2[:4]].tolist()

reduced_fullset_eval = X_reduced.drop(top5_rf_shap_features[0], axis=1)
X_cv4_rf_shap = reduced_fullset_eval[top4_rf_shap_features_reduced]
cv4_rf_shap_score = cross_val_score(rf_shap2, X_cv4_rf_shap, y_reduced, cv=5, scoring='r2').mean()

results['Method'].append('RF-SHAP')
results['CV5'].append(cv5_rf_shap_score)
results['CV4'].append(cv4_rf_shap_score)
results['Top5 Features'].append(', '.join(top5_rf_shap_features))
results['Top4 Features'].append(', '.join(top4_rf_shap_features_reduced))

# ─────────────────────────────────────────────
# 7. XGB-SHAP (using 100 instances)
# ─────────────────────────────────────────────
print("Running XGB-SHAP Feature Selection (100 instances)...")
xgb_shap = XGBRegressor(n_estimators=100, random_state=42)
...
xgb_shap_idx = np.argsort(mean_abs_shap_xgb)[::-1]
top5_xgb_shap_features = X_shap.columns[xgb_shap_idx[:5]].tolist()

# CV5 evaluation
X_cv5_xgb_shap = X_reduced[top5_xgb_shap_features]
cv5_xgb_shap_score = cross_val_score(xgb_shap, X_cv5_xgb_shap, y_reduced, cv=5, scoring='r2').mean()

# CV4: drop top feature and recalculate
reduced_fullset_xgb_shap = X_shap.drop(top5_xgb_shap_features[0], axis=1)

xgb_shap2 = XGBRegressor(n_estimators=100, random_state=42)
xgb_shap2.fit(reduced_fullset_xgb_shap, y_shap_reduced)

explainer_xgb2 = shap.TreeExplainer(xgb_shap2)
shap_values_xgb2 = explainer_xgb2.shap_values(reduced_fullset_xgb_shap)

mean_abs_shap_xgb2 = np.abs(shap_values_xgb2).mean(axis=0)
xgb_shap_idx2 = np.argsort(mean_abs_shap_xgb2)[::-1]
top4_xgb_shap_features_reduced = reduced_fullset_xgb_shap.columns[xgb_shap_idx2[:4]].tolist()

reduced_fullset_xgb_eval = X_reduced.drop(top5_xgb_shap_features[0], axis=1)
X_cv4_xgb_shap = reduced_fullset_xgb_eval[top4_xgb_shap_features_reduced]
cv4_xgb_shap_score = cross_val_score(xgb_shap2, X_cv4_xgb_shap, y_reduced, cv=5, scoring='r2').mean()

results['Method'].append('XGB-SHAP')
results['CV5'].append(cv5_xgb_shap_score)
results['CV4'].append(cv4_xgb_shap_score)
results['Top5 Features'].append(', '.join(top5_xgb_shap_features))
results['Top4 Features'].append(', '.join(top4_xgb_shap_features_reduced))
